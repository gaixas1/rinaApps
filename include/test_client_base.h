...
public:
	TestClientBase(const std::string Name, const std::string Instance, const std::string Servername, const std::string ServerInstance, const std::string DIF,
		int FlowIdent, int QosIdent, int TestDuration) :
		BaseClient(Name, Instance, Servername, ServerInstance, DIF) {
		_FlowIdent = _FlowIdent;
		_QosIdent = QosIdent;
...
protected:
	...
	int SendData(size_t Size, int mSec = -1) {
		...
		if (mSec < 0) {
			return ra::WriteData(_Fd, (ra::byte_t*) Data, Size);
		} else {
			return ra::WriteDataTimeout(_Fd, (ra::byte_t*) Data, Size, mSec);
		}
	}
